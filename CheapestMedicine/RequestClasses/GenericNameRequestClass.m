...
@implementation GenericNameRequestClass
@synthesize GenericSearchDelegate;
...
-(void)didfinishedParsing:(NSMutableDictionary *)parsedDictionary{
    ...
    [self.GenericSearchDelegate performSelectorOnMainThread:@selector(successJSONParsingGenericSearch:) withObject:parsedDictionary waitUntilDone:NO];
}

-(void)didparsingFailed{
    
    
    [self.GenericSearchDelegate performSelectorOnMainThread:@selector(failureJSONParsingGenericSearchResult) withObject:nil waitUntilDone:NO];
