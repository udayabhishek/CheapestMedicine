//
//  SearchByGenericViewController.m
//  CheapestMedicine
//
...
@interface SearchByGenericViewController ()

@end

@implementation SearchByGenericViewController
...
-(void)viewDidLoad
{
    ...
    if(UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad)
    {
        ...
        [genericTableView registerNib:[UINib nibWithNibName:@"MedicineSearch_iPad" bundle:nil] forCellReuseIdentifier:@"cell"];
        
    }
    else
    {
      [genericTableView registerNib:[UINib nibWithNibName:@"MedicineSearch" bundle:nil] forCellReuseIdentifier:@"cell"];
    }
    strMedicine=[[NSString alloc]init];
...
#pragma - method for sending request From server

-(void)makeRequestForMedicineSearchByName{
    spinnerView.hidden=NO;
     int points = (int)[[defaults objectForKey:@"Count" ]integerValue];
    ...
    //else if ([[defaults objectForKey:@"Count"] isEqualToString:@"20"]){
        ...
        [self hideBanners];
        GenericNameRequestClass *nameSearch=[[GenericNameRequestClass alloc]init];
        nameSearch.GenericSearchDelegate=self;
        [nameSearch makeRequestForCheckingwithGenericSearchtxt:searchMedicine.text Value:[NSString stringWithFormat:@"%i",count]];
    }
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DetailGenericViewController *detail;
    if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad) {
        detail=[[DetailGenericViewController alloc]initWithNibName:@"DetailGenericViewController_iPad" bundle:nil];
        
    }
    
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
        if (IS_IPHONE_5){
            detail=[[DetailGenericViewController alloc]initWithNibName:@"DetailGenericViewController" bundle:nil];
            
        }
        else{
            detail=[[DetailGenericViewController alloc]initWithNibName:@"DetailGenericViewController_iPhone" bundle:nil];
...
- (void)adViewWillLeaveApplication:(GADBannerView *)adView{
    [self hideBanners];
    int i = [[[NSUserDefaults standardUserDefaults] valueForKey:@"Count"] intValue];
